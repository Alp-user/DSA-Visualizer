...
mod dsas;
mod hashgrid;
use glfw::{fail_on_errors, Action, Context, GlfwReceiver, Key, OpenGlProfileHint, Window, WindowEvent, WindowHint, WindowMode};
...
pub fn resize_camera(width: f32, height: f32, cam_horizontal: f32, cam_vertical: f32) {
  unsafe {
    CAMERA_SHIFT = (CAMERA_SHIFT.0 + cam_horizontal, CAMERA_SHIFT.1 + cam_vertical);
...
fn main() {
  ...
  while !window.should_close() {
    ...
    if !unsafe { STABLE_HAPPENED } {
      unsafe { STABLE_HAPPENED = graph_draw.simulation_step(); }
    }
...
fn handle_window_event(events: &GlfwReceiver<(f64, WindowEvent)>, window: &mut glfw::PWindow) {
  for (_, event) in glfw::flush_messages(events) {
    match event {
      ...
      glfw::WindowEvent::Key(Key::Backspace, _, Action::Release, _) => {
        unsafe { ERASE = false; }
      },
